...
def walk_and_scan(root: Path, print_results: bool = False) -> tuple[int, int]:
    ...
    trace_file_suffix = ".dynamoscan.trace.json"
    report_file_suffix = ".dynamoscan.json"
    for trace_file in root.rglob(f"*{trace_file_suffix}"):
        total_files += 1

        # Derive model path by removing .trace.json suffix
        model_name = trace_file.name[:-len(f"{trace_file_suffix}")]
        model_path = trace_file.parent / model_name

        # Create report file path
        report_file = trace_file.with_name(model_name + report_file_suffix)
